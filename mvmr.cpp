...
// initialize
  ...
  for(unsigned int random_ind = 0; random_ind < random_start; ++random_ind)
  {
    ...
    Rcpp::List MLE_result = MVcML_estimate_c(b_exp,b_out,
                              ...
                              fix_bx,
                              thres); 
    // std::cout << as<int>(MLE_result["Converge"])<<std::endl;
...
// [[Rcpp::export]]
Rcpp::List MVmr_cML_c(const arma::mat& b_exp, const arma::mat& b_out,
                      const arma::mat& se_bx,
                      const Rcpp::List& Sig_inv_l,
                      const unsigned int n,
                      ...
                      const double thres = 1e-4)
{
  ...
  for(unsigned int i=0; i < K_vec_len; ++i)
  {
    ...
    Rcpp::List rand_res = MVcML_estimate_random_c(b_exp,b_out,se_bx,Sig_inv_l,
                                     WK1vec, WK1mat,
                                     K_value, n, 
                                     random_start, maxit,fix_bx,thres);
...
// [[Rcpp::export]]
Rcpp::List MVmr_cML_DP_c(const arma::mat& b_exp, const arma::mat& b_out,
                      const arma::mat& se_bx,
                      const Rcpp::List& Sig_inv_l,
                      const unsigned int n,
                      Rcpp::NumericVector K_vec = Rcpp::NumericVector::create(),
                      ...
                      const double thres = 1e-4)
{
  ...
  Rcpp::List cML_res = MVmr_cML_c(b_exp, b_out, se_bx, Sig_inv_l, n, K_vec, random_start, maxit, fix_bx, thres);
  ...
  for(unsigned int pt_ind = 0; pt_ind < num_pert; ++pt_ind) 
    ...
    for(unsigned int i = 0; i < p; ++i)
      ...
      Rcpp::List cML_res_b = MVmr_cML_c(b_exp_new, b_out_new, se_bx, Sig_inv_l, n, K_vec, random_start, maxit, fix_bx, thres);
