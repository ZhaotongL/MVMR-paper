...
for(j in (20*(array_id-1)):(20*array_id-1)+1){
  ...
  x3 = G%*%alpha  #new
  y = theta1*x1 + theta2*x2 + x3 + 0.5*c1 + 0.5*c2+  V[,1]
  ...
  x32 = G2%*%alpha#new
  y2 = theta1*x12 + theta2*x22 +  x32 + 0.5*c21 + 0.5*c22 + V2[,1]
...
#################################
 ...
 pihat = stderr = matrix(nrow = L, ncol = kx)
  ...
  correlation = matrix(c(1, cor(x1,x2), cor(x1,x2), 1), nrow = 2, ncol = 2)
    ...
    mvcmldp_res = MVmr_cML_DP_c(b_exp = simdat$bxhat,b_out = as.matrix(simdat$byhat),se_bx=simdat$sebx,Sig_inv_l,n=n,maxit=100,random_start=10,num_pert=100)
...
#    ivw_Q = qhet_mvmr(r_input=F.data, pcor=correlation, CI=TRUE, iterations=100)
